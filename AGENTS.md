...
* A skill's redirects, the tasks that belong to a sibling or lie outside the
  ...
  the model keeps working. A verb file's own redirects sit under
  `## Redirects` in that file. Keep a mid-procedure command of a sibling
  ("extract the paper with `/read-pdf`") in the procedure.
